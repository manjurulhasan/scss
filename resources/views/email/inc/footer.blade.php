<table width="100%" cellpadding="0" cellspacing="0" border="0" style="background: #000000;">
    <tr>
        <td width="48"></td>
        <td>
            <table width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr><td height="34"></td></tr>
                <tr>
                    <td>
                        <div style="font-weight: 600;font-size: 14px;letter-spacing: 0.02em;color: #FFFFFF;">SUPPORT</div>
                    </td>
                </tr>
                <tr><td height="8"></td></tr>
                <tr>
                    <td>
                        <div style="font-weight: 450;font-size: 14px;letter-spacing: 0.02em;color: #FFFFFF;">FAQs</div>
                    </td>
                </tr>
                <tr><td height="8"></td></tr>
                <tr>
                    <td>
                        <div style="font-weight: 450;font-size: 14px;letter-spacing: 0.02em;color: #FFFFFF; display: flex; align-items: center;">
                            <img src="{{ asset('assets/images/phone.png') }}"  width="16" height="16" alt="">
                            <span style="margin-left: 8px;"> +8801792221349 </span>
                        </div>
                    </td>
                </tr>
                <tr><td height="8"></td></tr>
                <tr>
                    <td>
                        <a href="mailto:bestellung@vicafe.ch">
                            <div style="font-weight: 450;font-size: 14px;letter-spacing: 0.02em;color: #FFFFFF; display: flex; align-items: center;">
                                <img src="{{ asset('assets/images/mail.png') }}"  width="16" height="16" alt="">
                                <span style="margin-left: 8px; color: #ffffff;text-decoration: none;"> information.scss@gmail.com</span>
                            </div>
                        </a>
                    </td>
                </tr>
                <tr><td height="8"></td></tr>
                <tr>
                    <td>
                        <div style="font-weight: 450;font-size: 14px;letter-spacing: 0.02em;color: #FFFFFF; display: flex; align-items: center;">
                            <img src="{{ asset('assets/images/map-pin.png') }}"  width="16" height="16" alt="">
                            <span style="margin-left: 8px;"> Dhaka, Bangladesh </span>
                        </div>
                    </td>
                </tr>
                <tr><td height="33"></td></tr>
            </table>
        </td>
        <td valign="top">
            <table width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr><td height="34"></td></tr>
                <tr>
                    <td align="right">
                        <div style="font-weight: 600;font-size: 14px;letter-spacing: 0.02em;color: #FFFFFF;">FOLGE UNS</div>
                    </td>
                </tr>
                <tr><td height="8"></td></tr>
                <tr>
                    <td align="right">
                        <table>
                            <tr>
                                <td>
                                    <a href="#">
                                        <img src="{{ asset('assets/images/facebook.png') }}" width="20" height="20" alt="">
                                    </a>
                                </td>
                                <td width="2"></td>
                                <td>
                                    <a href="#">
                                        <img src="{{ asset('assets/images/instagram.png') }}" width="20" height="20" alt="">
                                    </a>
                                </td>
                                <td width="2"></td>
                                <td>
                                    <a href="#">
                                        <img src="{{ asset('assets/images/linkedin.png') }}" width="20" height="20" alt="">
                                    </a>
                                </td>
                            </tr>
                        </table>




                    </td>
                </tr>

                <tr><td height="33"></td></tr>
            </table>
        </td>
        <td width="48"></td>
    </tr>
</table>
<!--last part ends-->

</div>
</body>

</html>
